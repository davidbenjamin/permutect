...
class MonoDenseLayer(nn.Module):
    ...
    def __init__(
        ...
        self.free_W = (
            nn.Parameter(torch.empty((output_dimension, input_dimension - self.num_constrained)))
            if num_free > 0
            else None
        )
...
class MonotonicHighwayLayer(nn.Module):
    ...
    def __init__(self, dim: int, num_layers: int):
        ...
        self.nonlinear = MonoDense(
            input_dimension=dim,
            output_dimensions=(num_layers * [dim]),
            num_increasing=dim,
            num_decreasing=0,
        )
