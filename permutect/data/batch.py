...
from permutect.data.datum import Datum, FLOAT_TO_LONG_MULTIPLIER
...
class Batch:
    ...
    def get_info_be(self) -> Tensor:
        return self.data[:, self.info_start:self.info_end] / FLOAT_TO_LONG_MULTIPLIER

    # this is -log10ToLog(TLOD) - log(tumorDepth + 1);
    def get_seq_error_log_lks(self) -> Tensor:
        return self.data[:, Datum.SEQ_ERROR_LOG_LK_IDX] / FLOAT_TO_LONG_MULTIPLIER

    # this is -log10ToLog(NALOD) - log(normalDepth + 1)
    def get_normal_seq_error_log_lks(self) -> Tensor:
        return self.data[:, Datum.NORMAL_SEQ_ERROR_LOG_LK_IDX] / FLOAT_TO_LONG_MULTIPLIER
