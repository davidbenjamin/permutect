...
from permutect.data.read_set import ReadSet, ReadSetBatch, load_list_of_read_sets, CountsAndSeqLks, \
    RepresentationReadSet, RepresentationReadSetBatch
...
class RepresentationDataset(Dataset):
    def __init__(self, read_set_dataset: ReadSetDataset, representation_model: RepresentationModel):
        ...
        for read_set_batch in loader:
            representations = representation_model.calculate_representations(read_set_batch)
...
class SemiSupervisedRepresentationBatchSampler(Sampler):
    ...
    def __iter__(self):
        batches = []    # list of lists of indices -- each sublist is a batch
        for index_list in chain(self.labeled_indices, self.unlabeled_indices):
            random.shuffle(index_list)
            batches.extend(chunk(index_list, self.batch_size))
        random.shuffle(batches)
