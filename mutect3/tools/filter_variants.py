...
def make_posterior_data_loader(dataset_file, input_vcf, artifact_model: ArtifactModel, batch_size: int, chunk_size: int):
    ...
    for list_of_read_sets in plain_text_data.generate_normalized_data([dataset_file], chunk_size, include_variant_string=True):
        ...
        all_folds = list(range(artifact_dataset.num_folds))
        artifact_loader = read_set_dataset.make_data_loader(artifact_dataset, all_folds, batch_size, pin_memory=False, num_workers=0)
